...
DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parent / "outputs_lstm"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train LSTM model for customer behavior prediction.")
...
def save_plots(output_dir: Path, history: Dict[str, List[float]], y_true: Sequence[int], y_pred: Sequence[int], labels: Sequence[str]) -> None:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "lstm_loss.png", dpi=160)
    plt.close()
    ...
    plt.colorbar()
    plt.tight_layout()
    plt.savefig(output_dir / "lstm_confusion_matrix.png", dpi=160)
    plt.close()
...
def main() -> None:
    ...
    print(f"LSTM test macro F1: {metrics['f1_macro']:.4f}")
