...
DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parent / "outputs_rnn"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train RNN model for customer behavior prediction.")
...
def save_plots(output_dir: Path, history: Dict[str, List[float]], y_true: Sequence[int], y_pred: Sequence[int], labels: Sequence[str]) -> None:
    ...
    plt.title("RNN Train/Validation Loss")
    plt.xlabel("Epoch")
    plt.ylabel("Cross entropy loss")
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "rnn_loss.png", dpi=160)
    plt.close()
    ...
    plt.colorbar()
    plt.tight_layout()
    plt.savefig(output_dir / "rnn_confusion_matrix.png", dpi=160)
    plt.close()
...
def main() -> None:
    ...
    print(f"RNN test macro F1: {metrics['f1_macro']:.4f}")
